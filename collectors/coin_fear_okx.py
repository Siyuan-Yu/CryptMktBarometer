...
接入步骤：
1. 在 config api_keys.okx 填入 Key
2. 设置 data_sources.okx_fear.enabled: true
3. 实现 fetch_okx_sol_fear_latest / fetch_okx_sol_fear_history
4. 在 OkxSolFearProvider 中取消 TODO 注释
"""
...
def fetch_okx_sol_fear_latest(
    trade_date: str,
    *,
    cfg: dict | None = None,
) -> FearSample | None:
    """拉取 OKX SOL 当日恐慌分（待对接）。"""
    settings = _okx_settings(cfg)
    if not settings["enabled"] or not settings["base_url"]:
        return None
    # TODO: OKX API 请求与字段映射
    return None


def fetch_okx_sol_fear_history(
    *,
    start_date: str,
    end_date: str,
    cfg: dict | None = None,
